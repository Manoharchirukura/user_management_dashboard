...
const UserList = () => {
    ...
    useEffect(() => {
        // This function fetches the users from your live backend on Render
        const fetchUsers = async () => {
            try {
                // This line makes the API call and correctly assigns the result to 'response'
                const response = await axios.get('https://user-management-dashboard-87fp.onrender.com/api/users');
                
                // This is the safeguard to prevent crashes if the API response is not what we expect
                setUsers(response.data.data || []);
            } catch (error) {
                console.error("There was an error fetching the users!", error);
                // In case of an error, we'll set users to an empty array to prevent a crash
                setUsers([]);
            }
        };
    ...
    const handleDelete = async (userId) => {
        ...
        if (confirmDelete) {
            try {
                await axios.delete(`https://user-management-dashboard-87fp.onrender.com/api/users/${userId}`);
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                    {users.length > 0 ? (
                        ...
                        <tr>
                            <td colSpan="4" style={{ padding: '12px', textAlign: 'center' }}>No users found or data is loading...</td>
                        </tr>
                    )}
                </tbody>
            </table>
